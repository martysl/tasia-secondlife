...
void process_improved_im(LLMessageSystem *msg, void **user_data)
{
    ...
    {
        // <Tasia> DIAG: log EVERY IM unconditionally (no throttle) so no tip IM
        // can slip past the diagnostic.
        ...
        if (tipThankEnabled && boost::regex_search(message, match, tipped_regex) && match.size() >= 3)
        {
            ...
            if (tip_throttle.getElapsedTimeF32() > 10.f)
            {
                tip_throttle.reset();
                ...
                // message already appears as a normal say — an echo is redundant.
                if ((S32)tipThankChannel != 0 && FSFloaterNearbyChat::findInstance())
                {
                    LLChat visible_chat;
...
// [/RLVa:KB]
    ...
    {
        static LLCachedControl<bool> tipThankEnabled(gSavedSettings, "TasiaTipThankEnabled");
        // <Tasia> DIAG: log EVERY object chat unconditionally (no throttle) so
        // no tip message can slip past the diagnostic.
        std::string tip_mesg;
        ...
        if (tipThankEnabled)
        {
            ...
            if (boost::regex_search(tip_mesg, match, tipped_regex) && match.size() >= 3)
            {
                ...
                if (tip_chat_throttle.getElapsedTimeF32() > 10.f)
                {
